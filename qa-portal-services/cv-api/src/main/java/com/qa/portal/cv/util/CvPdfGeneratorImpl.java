...
public class CvPdfGeneratorImpl implements CvPdfGenerator {
	...
	public void generateConsultantNameBox(CvVersion cvVersion) throws IOException {
		CvPdfElement consultantNameBox = new CvPdfElement(ConsultantNameBox.WIDTH.value, ConsultantNameBox.HEIGHT.value,
	...
	public void generateSkillsBox(CvVersion cvVersion) throws IOException {
		CvPdfElement skillsBox = new CvPdfElement(SkillsBox.WIDTH.value, SkillsBox.HEIGHT.value,
				SkillsBox.X_POSITION.value, SkillsBox.Y_POSITION.value);
		paragraph = skillsBox.getParagraph();
		frame = skillsBox.getFrame();
		generateSkillsBoxContent(paragraph, "Programming Languages",
	...
	public void generateSkillsBoxContent(Paragraph paragraph, String title, List<String> list) throws IOException {
		...
		for (int i = 0; i < list.size(); i++) {
			if (i < list.size() - 1) {
				paragraph.addMarkup("{color:#FFFFFF}" + list.get(i) + ", ", FontSize.SIDEBAR_CONTENT.value, montserrat,
						montserratBold, montserrat, montserrat);
			} else {
	...
	public void generateQualificationsBox(CvVersion cvVersion) throws IOException {
		CvPdfElement qualificationsBox = new CvPdfElement(QualificationsBox.WIDTH.value, QualificationsBox.HEIGHT.value,
		...
		for (Qualification i : cvVersion.getAllQualifications()) {
			paragraph.addMarkup("{color:#FFFFFF}" + i.getQualificationDetails(), FontSize.SIDEBAR_CONTENT.value,
					montserrat, montserratBold, montserrat, montserrat);
		}
	...
	public void generateMainHeader(CvVersion cvVersion) throws IOException {
		CvPdfElement header = new CvPdfElement(MainHeader.WIDTH.value, MainHeader.HEIGHT.value,
	...
	public void generateMainBody(CvVersion cvVersion) throws IOException {
		CvPdfElement body = new CvPdfElement(MainBody.WIDTH.value, MainBody.HEIGHT.value, MainBody.X_POSITION.value,
		...
		for (int i = 0; i < cvVersion.getAllWorkExperience().size(); i++) {
			paragraph.addMarkup(
					"{color:" + ColourScheme.QA_BLUE.value + "}*"
							+ cvVersion.getAllWorkExperience().get(i).getJobTitle() + "*\n",
					FontSize.BODY_CONTENT.value, montserrat, montserratBold, montserrat, montserrat);
	...
	public void generateMainFooter(CvVersion cvVersion) throws IOException {
		CvPdfElement footer = new CvPdfElement(MainFooter.WIDTH.value, MainFooter.HEIGHT.value,
	...
	public void divider(float yPosition) throws IOException {
		CvPdfElement divider = new CvPdfElement(Dividers.WIDTH.value, Dividers.HEIGHT.value, Dividers.X_POSITION.value,
				yPosition);
